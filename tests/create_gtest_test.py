...
def _cmake_build(folder: pathlib.Path, output: pathlib.Path) -> None: # (or built binaries?) list[pathlib.Path]:
    ...
    with tempfile.TemporaryDirectory() as build:
        ...
        cmake_config = subprocess.run((
            "cmake", "-B", build, "-S", folder.absolute(), "--fresh", f"-D EXECUTABLE_OUTPUT_PATH={output}",
        ...
        cmake_build = subprocess.run(("cmake", "--build", build, "--target", TEST_TARGET))
...
elseif(EXISTS ${{_FALLBACK_GTEST_DIR}})
   ...
   FetchContent_Declare(
      googletest
      BINARY_DIR ${{_FALLBACK_GTEST_DIR}}/gtest-build
      SOURCE_DIR ${{_FALLBACK_GTEST_DIR}}/gtest-src
   )
   FetchContent_MakeAvailable(googletest)
else()
